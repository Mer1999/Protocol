...
#define inc(k) if(k<MAX_SEQ) k=k+1; else k=0; //使k在[0 ~ MAX_SEQ-1]间循环增长,如果MAX_SEQ=1，则0/1互换#pragma once
#include<iostream>
using namespace std;
#define MAX_PKT 1024 //每一帧最大容量
#define MAX_FILE_LEN 128 //最大共享文件名长度
...
/*使网络层阻塞,不再产生新的network_layer_ready事件*/
void disable_network_layer(void);
#define inc(k) if(k<MAX_SEQ) k=k+1; else k=0; //使k在[0 ~ MAX_SEQ-1]间循环增长,如果MAX_SEQ=1，则0/1互换
